...
import static io.vertx.guides.wiki.DatabaseConstants.CONFIG_WIKIDB_JDBC_DRIVER_CLASS;
import static io.vertx.guides.wiki.DatabaseConstants.CONFIG_WIKIDB_JDBC_MAX_POOL_SIZE;
import static io.vertx.guides.wiki.DatabaseConstants.CONFIG_WIKIDB_JDBC_URL;
import static io.vertx.guides.wiki.DatabaseConstants.DEFAULT_JDBC_MAX_POOL_SIZE;
import static io.vertx.guides.wiki.DatabaseConstants.DEFAULT_WIKIDB_JDBC_DRIVER_CLASS;
import static io.vertx.guides.wiki.DatabaseConstants.DEFAULT_WIKIDB_JDBC_URL;
...
public class HttpServerVerticle extends AbstractVerticle {
	...
	public void start(Promise<Void> promise) throws Exception {
		...
		router.get("/app/*").handler(StaticHandler.create().setAllowRootFileSystemAccess(true)
				.setWebRoot("./webroot/app").setCachingEnabled(false)).failureHandler((ctx) -> {
					LOGGER.error("ne peut pas servir app", ctx.failure());
				});
		router.get("/").handler(context -> context.reroute("/app/index.html")).failureHandler((ctx) -> {
			LOGGER.error("pas pu afficher index.html", ctx.failure());
		});
	...
	private void indexHandler(RoutingContext context) {
		context.user().rxIsAuthorized("create").flatMap(canCreatePage -> {
	...
	private void pageRenderingHandler(RoutingContext context) {
		...
		user.rxIsAuthorized("update").flatMap(canSavePage -> {
	...
	private void pageUpdateHandler(RoutingContext context) {
		...
		checkAuthorised(context, pageCreation ? "create" : "update")
				.andThen(pageCreation ? dbService.rxCreatePage(title, markdown)
						: dbService.rxSavePage(Integer.valueOf(context.request().getParam("id")), markdown))
				.subscribe(() -> {
	...
	private void pageDeletionHandler(RoutingContext context) {
		checkAuthorised(context, "delete")
				.andThen(dbService.rxDeletePage(Integer.valueOf(context.request().getParam("id")))).subscribe(() -> {
					context.response().setStatusCode(303);
	...
	private void backupHandler(RoutingContext context) {
		checkAuthorised(context, "role:writer").andThen(dbService.rxFetchAllPagesData()).map(pages -> {
			...
			return payload;
		}).flatMap(body -> webClient.post(443, "snippets.glot.io", "/snippets").putHeader("User-Agent", "vert-x3")
				.putHeader("Content-Type", "application/json").as(BodyCodec.jsonObject()).rxSendJsonObject(body))
				.subscribe(response -> {
